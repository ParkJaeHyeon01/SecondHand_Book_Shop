...
const SellingList = ({ route, navigation }) => {
  const { sellingData, selldata } = route.params;

  // 최신순 정렬
  const onSaleBooks = [...sellingData.on_sale_book_list].sort(
    ...
    >
      <BookCard>
        ...
        <BookInfo>
          ...
          <BookDate>등록일 : {new Date(item.createAt).toLocaleDateString()}</BookDate>
        </BookInfo>
      </BookCard>
    </TouchableOpacity>
  );
  ...
  return (
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginRight:190 }}>
                  <TouchableOpacity onPress={goToHome}>
                    <Ionicons name="chevron-back-outline" size={30} color="black" style={{ marginLeft: 10 }} />
                  </TouchableOpacity>
                  <Text style={{ fontWeight: 'bold', fontSize: 28, marginLeft:10, width:180 }}>판매 중 리스트</Text>
                </View>
        <Container>
      <FlatList
        data={onSaleBooks}
