...
const ShopAdjustmentList = () => {
  const route = useRoute();
  const navigation = useNavigation();
  
  const { payment_group,shop_info } = route.params;
  const shopId = shop_info.shopId
  
  const adjustmentList = payment_group || []; 
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/shop/${userId}/${shopId}/check-payment-req/${pyid}`, {
      method: 'GET',
      headers: {
    ...
    console.log(result);
    navigation.navigate("ShopAdjustmentDetail", result)
  } catch (error) {
  ...
  const renderItem = ({ item }) => { 
    ...
    const firstBookTitle = item.books && item.books.length > 0 ? item.books[0].title : '책 정보 없음';
    const bookCount = item.books ? item.books.length : 0;
    
    const displayTitle = bookCount > 1 ? `${firstBookTitle} 외 ${bookCount - 1}권` : firstBookTitle;
...
export default ShopAdjustmentList;
